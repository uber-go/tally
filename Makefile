...
PKGS     = $(or $(PKG),$(shell env GO111MODULE=on $(GO) list ./... | grep -v thirdparty))
...
lint: | $(GOLINT) ; $(info $(M) running golint…) @ ## Run golint
	$Q $(GOLINT) $(PKGS) 2>&1 | grep -v '$(LINT_IGNORE)' | tee -a lint.log
...
fmt: ; $(info $(M) running gofmt…) @ ## Run gofmt on all source files
	$Q $(GO) fmt $(PKGS) 2>&1 | grep -v '$(LINT_IGNORE)' | tee lint.log
