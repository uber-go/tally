...
func BenchmarkNameGeneration(b *testing.B) {
	scope := NewRootScope("funkytown", nil, NullStatsReporter, 0).(*standardScope)
	for n := 0; n < b.N; n++ {
...
func BenchmarkNameGenerationTagged(b *testing.B) {
	...
	scope := NewRootScope("funkytown", tags, NullStatsReporter, 0).(*standardScope)
	for n := 0; n < b.N; n++ {
		scope.fullyQualifiedName("take.me.to")
	}
}
...
func BenchmarkNameGenerationNoPrefix(b *testing.B) {
	scope := NewRootScope("", nil, NullStatsReporter, 0).(*standardScope)
	for n := 0; n < b.N; n++ {
