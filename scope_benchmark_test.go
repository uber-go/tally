...
func BenchmarkNameGeneration(b *testing.B) {
	scope := NewScope("funkytown", nil, NullStatsReporter, 0).(*standardScope)
	for n := 0; n < b.N; n++ {
...
func BenchmarkNameGenerationTagged(b *testing.B) {
	...
	scope := NewScope("funkytown", tags, NullStatsReporter, 0).(*standardScope)
	for n := 0; n < b.N; n++ {
		scope.fullyQualifiedName("take.me.to")
	}
}
...
func BenchmarkNameGenerationNoPrefix(b *testing.B) {
	scope := NewScope("", nil, NullStatsReporter, 0).(*standardScope)
	for n := 0; n < b.N; n++ {
